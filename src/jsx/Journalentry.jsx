...
function pdfTxt(page, text, x, y, size, options = {}) {
  ...
  page.push(`q ${gray} g BT /${font} ${size} Tf ${tx.toFixed(2)} ${y.toFixed(2)} Td (${safe}) Tj ET Q`);
}
...
function drawWrappedPdfText(page, text, x, y, size, maxWidth, maxLines, options = {}) {
  ...
  lines.forEach((lineText, index) => {
    pdfTxt(page, lineText, x, y - index * 9, size, options);
  });
...
function buildJournalVoucherPdfBlob({ header, lines, totals }) {
  ...
  const address   = String(header.companyAddress || '');
  ...
  const entryDate = formatDisplayDate(header.transactionDate);
  ...
  const codeW  = PW * 0.16;
  const titleW = PW * 0.54;
  const amtW   = PW * 0.15;
  const COLS   = [M, M + codeW, M + codeW + titleW, M + codeW + titleW + amtW];
  ...
  pdfTxt(stream, company, M, top, 8, { font: 'F2', gray: 0.1, align: 'center', width: PW });
  top -= 14;
  pdfHLine(stream, M, top, M + PW);
  top -= 10;

  if (address) { pdfTxt(stream, address, M, top, 7, { gray: 0.2, align: 'center', width: PW }); top -= 10; }
  if (tin)     { pdfTxt(stream, `TIN #: ${tin}`, M, top, 7, { gray: 0.2, align: 'center', width: PW }); top -= 10; }

  top -= 6;
  pdfTxt(stream, 'JOURNAL VOUCHER', M, top, 14, { font: 'F2', gray: 0.08, align: 'center', width: PW });
  top -= 18;

  const metaY = top;
  const noteLabelX = M;
  const noteX = M + 80;
  const metaRightX = M + PW - 245;
  const metaValueX = metaRightX + 86;
  const noteMaxWidth = metaRightX - noteX - 18;
  const metaValueMaxWidth = M + PW - metaValueX;
  pdfTxt(stream, 'Notes', noteLabelX, metaY, 7, { font: 'F2', gray: 0.1 });
  drawWrappedPdfText(stream, note, noteX, metaY, 7, noteMaxWidth, 3, { gray: 0.1 });
  pdfTxt(stream, 'Reference No.:', metaRightX, metaY, 7, { font: 'F2', gray: 0.1 });
  const referenceLineCount = drawWrappedPdfText(stream, refNo, metaValueX, metaY, 7, metaValueMaxWidth, 3, { gray: 0.1 });
  const entryY = metaY - Math.max(11, referenceLineCount * 9);
  pdfTxt(stream, 'Entry No.:', metaRightX, entryY, 7, { font: 'F2', gray: 0.1 });
  const entryLineCount = drawWrappedPdfText(stream, entryNo, metaValueX, entryY, 7, metaValueMaxWidth, 2, { gray: 0.1 });
  const dateY = entryY - Math.max(11, entryLineCount * 9);
  pdfTxt(stream, 'Date:', metaRightX, dateY, 7, { font: 'F2', gray: 0.1 });
  drawWrappedPdfText(stream, entryDate, metaValueX, dateY, 7, metaValueMaxWidth, 1, { gray: 0.1 });
  pdfTxt(stream, `Printed: ${printed}`, M, metaY - 22, 6, { gray: 0.45 });
  top -= 36;

  pdfFill(stream, M, top - hdrH, PW, hdrH, 0.94);
  pdfBorder(stream, M, top - hdrH, PW, hdrH);
  ['Account Code', 'Account Title', 'Debit', 'Credit'].forEach((label, i) => {
    if (i > 0) pdfBorder(stream, COLS[i], top - hdrH, WIDTHS[i], hdrH);
    pdfTxt(stream, label.toUpperCase(), COLS[i] + 4, top - 12, 6.5, { font: 'F2', gray: 0.15, align: i >= 2 ? 'center' : 'left', width: WIDTHS[i] - 8 });
  });
  top -= hdrH;

  activeLines.forEach((line, idx) => {
    const fill   = idx % 2 === 0 ? 1 : 0.975;
    const debit  = parseAmount(line.debit);
    const credit = parseAmount(line.credit);
    const sub    = line.subsidiary ? `  (${String(line.subsidiary)})` : '';

    pdfFill(stream, M, top - rowH, PW, rowH, fill);
    pdfBorder(stream, M,       top - rowH, codeW, rowH);
    pdfBorder(stream, COLS[1], top - rowH, titleW, rowH);
    pdfBorder(stream, COLS[2], top - rowH, amtW,  rowH);
    pdfBorder(stream, COLS[3], top - rowH, amtW,  rowH);

    pdfTxt(stream, String(line.accountCode || '-'), M + 4, top - 11, 6.5, { gray: 0.1 });
    pdfTxt(stream, String(line.accountTitle || '-') + sub, COLS[1] + 4, top - 11, 6.5, { gray: 0.1 });
    if (debit  > 0) pdfTxt(stream, formatMoney(debit),  COLS[2] + 4, top - 11, 6.5, { gray: 0.08, align: 'right', width: amtW - 8 });
    if (credit > 0) pdfTxt(stream, formatMoney(credit), COLS[3] + 4, top - 11, 6.5, { gray: 0.08, align: 'right', width: amtW - 8 });
    top -= rowH;
  });

  pdfFill(stream, M, top - hdrH, PW, hdrH, 0.92);
  pdfBorder(stream, M, top - hdrH, PW, hdrH);
  pdfTxt(stream, 'TOTAL >>', COLS[1] + 4, top - 12, 6.5, { font: 'F2', gray: 0.1, align: 'right', width: titleW - 8 });
  pdfTxt(stream, formatMoney(totals.debit),  COLS[2] + 4, top - 12, 7, { font: 'F2', gray: 0.05, align: 'right', width: amtW - 8 });
  pdfTxt(stream, formatMoney(totals.credit), COLS[3] + 4, top - 12, 7, { font: 'F2', gray: 0.05, align: 'right', width: amtW - 8 });
  top -= hdrH + 30;

  const sigW = PW / 4;
  ['Prepared By:', 'Checked By:', 'Approved By:', 'Received By:'].forEach((label, i) => {
    const sx = M + i * sigW;
    pdfHLine(stream, sx + 4, top, sx + sigW - 8);
    pdfTxt(stream, label, sx + 4, top - 12, 6.5, { gray: 0.3 });
  });
  ...
  useEffect(() => {
    ...
    const loadRows = async () => {
      ...
      try {
        ...
        if (sortConfig.key) {
          params.set('sortColumn', sortConfig.key);
          params.set('sortDirection', sortConfig.direction || 'asc');
        }
        ...
        const [journalResponse, bookResponse, employeeResponse] = await Promise.all([
          fetch(buildApiUrl(pagedUrl), { headers, signal: controller.signal }),
    ...
    return () => controller.abort();
  }, [page, normalizedQuery, sortConfig.key, sortConfig.direction]);
